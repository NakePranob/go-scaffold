...
function openapiLines(naming: ModuleNaming) {
  return {
    paths: [
      `/v1/${naming.plural}:`,
...
export function patchOpenapiIndex(openapiPath: string, naming: ModuleNaming): void {
  ...
  const { paths, schemas } = openapiLines(naming);
...
export function unpatchOpenapiIndex(openapiPath: string, naming: ModuleNaming): void {
